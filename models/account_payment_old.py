                    ...
                    if payment.sales_team_id.commission_range_ids:
                        ...
                        for range in payment.sales_team_id.commission_range_ids:
                            if commission_type == 'per_unit':
                                ...
                                # sales_person_commission =  (payment.amount * range.sales_person_commission)/100
                                sales_manager_commission = (payment.ownership_line_id.loan_id.pricing * range.sales_manager_commission) / 100
                                sales_person_commission = (payment.ownership_line_id.loan_id.pricing * range.sales_person_commission) / 100
                                # sales_broker_commission = range.broker_commission
                                # sales_party_commission = range.third_party_commission
                                sales_broker_commission = (payment.amount * range.broker_commission)/100
                                sales_party_commission = (payment.amount * range.third_party_commission)/100
            ...
            for payment in self:
                if payment.sales_commission_apply:
                    ...
                    if commission_based_on == 'sales_team':
                        ...
                        print(f'in invoice_payment an account payment commission - sales_team -amount_person: {amount_person} - amount_manager:{amount_manager} - broker_commission :{broker_commission} - third_party_commission:{third_party_commission} ')
